...
STRICT SEQUENTIAL CURSOR VERSION (PERSISTENT BIGQUERY SYNC STATE)
Targeting:
...
Uses persistent cursor `secop.sync_state` for 100% strict sequential ingestion without gaps or repetition.
...
SYNC_ID = "ingest_anexos_3_sectores"
...
TARGET_SECTOR_NITS = {
    # 1. Sector Defensa
    "899999003", "899999162", "830040256", "899999044", "860021967", "860021180", "860021653", "860020227", "800217123",
    # 2. Sector TIC
    "899999053", "900334265", "830002593", "901144049", "8001316486", "800131648", "900002583", "900062917", "830079479",
    # 3. Sector Inclusión Social y Reconciliación
    "900097800", "900490473", "900492970"
}
...
def get_sync_cursor():
    """Retrieve current sequential offset cursor from BigQuery secop.sync_state."""
    bq_client = get_bq_client()
    sql = f"""
    SELECT records_synced 
    FROM `{PROJECT}.{DATASET}.sync_state`
    WHERE sync_id = '{SYNC_ID}'
    """
    try:
        rows = list(bq_client.query(sql).result())
        if rows:
            return int(rows[0].records_synced)
    except Exception:
        pass
    return 0


def update_sync_cursor(new_offset):
    """Update sequential offset cursor in BigQuery secop.sync_state."""
    bq_client = get_bq_client()
    sql = f"""
    UPDATE `{PROJECT}.{DATASET}.sync_state`
    SET records_synced = {new_offset},
    ...
    try:
        ...
        print(f"Error actualizando cursor sync_state: {e}", flush=True)


def fetch_national_documents(offset=0, limit=200):
    """Fetch PDFs strictly for targeted sectors starting at exact sequential offset."""
...
def download_pdf_text(doc_url):
    """Download PDF and extract text using PyPDF with strict size and page caps."""
    try:
...
def run(max_documents=200, max_workers=5):
    ...
    base_cursor = get_sync_cursor()
    task_offset = base_cursor + (task_idx * max_documents)
    
    print(f"=== INICIANDO INGESTA SECUENCIAL ENFOCADA (TAREA #{task_idx} - CURSOR OFFSET #{task_offset:,}) ===", flush=True)
    
    docs = fetch_national_documents(offset=task_offset, limit=max_documents)
    print(f"Encontrados {len(docs)} documentos en offset #{task_offset:,}.", flush=True)
    
    processed_count = 0
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(process_single_document, doc): doc for doc in docs}
    ...
    # Task #0 advances persistent sync cursor for the next execution run
    if task_idx == 0:
        total_tasks = 4
        new_cursor = base_cursor + (total_tasks * max_documents)
        update_sync_cursor(new_cursor)
        print(f"=== CURSOR SECUENCIAL ACTUALIZADO EN BIGQUERY: Siguiente Offset #{new_cursor:,} ===", flush=True)
